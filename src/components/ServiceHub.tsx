...
import { 
  ...
  Clock, FileSearch, CheckCheck, XCircle, AlertCircle
} from "lucide-react";
...
export function RequestStatusBadge({ status, className = "", showIcon = true, size = "md" }: RequestStatusBadgeProps) {
  const sizeClasses = {
  ...
  if (status === RequestStatus.COMPLETED || status === "Đã hoàn thành" || status === "Đã giải quyết") {
    return (
      <span className={`inline-flex items-center gap-1.5 rounded-full font-extrabold bg-emerald-50 text-emerald-700 border border-emerald-200/90 shadow-2xs ${sizeClasses[size]} ${className}`}>
        {showIcon && <CheckCheck className={`${iconClass} text-emerald-600`} />}
        <span>Đã hoàn thành</span>
      </span>
    );
  }

  if (status === RequestStatus.APPROVED || status === "Đã duyệt hỗ trợ" || status === "Đã phê duyệt" || status === "Đã duyệt") {
    return (
      <span className={`inline-flex items-center gap-1.5 rounded-full font-extrabold bg-emerald-500/10 text-emerald-700 border border-emerald-500/30 shadow-2xs ${sizeClasses[size]} ${className}`}>
        {showIcon && <CheckCircle2 className={`${iconClass} text-emerald-600`} />}
        <span>Đã phê duyệt</span>
      </span>
    );
  }

  if (status === RequestStatus.VERIFYING || status === "Đang xác minh" || status === "Đang xử lý") {
    return (
      <span className={`inline-flex items-center gap-1.5 rounded-full font-extrabold bg-sky-50 text-sky-700 border border-sky-200/90 shadow-2xs ${sizeClasses[size]} ${className}`}>
        {showIcon && <FileSearch className={`${iconClass} text-sky-600 animate-pulse`} />}
        <span>Đang xác minh</span>
      </span>
    );
  }

  if (status === RequestStatus.REJECTED || status === "Từ chối" || status === "Từ chối giải quyết" || status === "Không đủ điều kiện") {
    return (
      <span className={`inline-flex items-center gap-1.5 rounded-full font-extrabold bg-rose-50 text-rose-700 border border-rose-200/90 shadow-2xs ${sizeClasses[size]} ${className}`}>
        {showIcon && <XCircle className={`${iconClass} text-rose-600`} />}
        <span>Từ chối</span>
      </span>
    );
  }

  // Default / SUBMITTED ("Đã tiếp nhận")
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full font-extrabold bg-amber-50 text-amber-700 border border-amber-200/90 shadow-2xs ${sizeClasses[size]} ${className}`}>
      {showIcon && <Clock className={`${iconClass} text-amber-600`} />}
      <span>{status || "Đã tiếp nhận"}</span>
    </span>
  );
}
                          ...
                          >
                            ...
                            <div className="py-3 px-2 bg-slate-50/50 rounded-2xl border border-slate-200/80 my-3 text-left">
                              ...
                              <div className="mt-3 pt-2 border-t border-slate-200/50 text-[10px] text-slate-500 font-light leading-normal bg-white/40 p-2 rounded-xl border border-slate-100">
                                ...
                                {r.status === RequestStatus.REJECTED && "Hồ sơ chưa đủ điều kiện tiếp nhận theo quy định hiện hành. Vui lòng xem phản hồi từ cán bộ phường bên dưới."}
                              </div>
                            </div>
                                          ...
                                          return (
                                            <motion.div
                                              ...
                                              className={`p-6 bg-white border rounded-2xl shadow-sm relative overflow-hidden transition-all duration-300 hover:shadow-md border-l-4 ${
                                                r.status === RequestStatus.COMPLETED ? "border-l-emerald-500 border-slate-100 hover:border-emerald-500/40 bg-emerald-50/[0.01]" :
                                                r.status === RequestStatus.APPROVED ? "border-l-emerald-600 border-slate-100 hover:border-emerald-500/40 bg-emerald-50/[0.01]" :
                                                r.status === RequestStatus.VERIFYING ? "border-l-sky-500 border-slate-100 hover:border-sky-300/40 bg-sky-50/[0.01]" :
                                                r.status === RequestStatus.REJECTED ? "border-l-rose-500 border-rose-200/80 hover:border-rose-300 bg-rose-50/[0.02]" :
                                                "border-l-amber-500 border-slate-100 hover:border-amber-300/40 bg-amber-50/[0.01]"
                                              }`}
                                            >
                                              {/* Card Header */}
